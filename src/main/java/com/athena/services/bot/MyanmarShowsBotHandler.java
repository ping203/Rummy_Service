...
import com.dst.config.myanShows.ShowsConstant;
...
public class MyanmarShowsBotHandler implements Bot {
    public final static Integer GAMEID = com.athena.services.utils.GAMEID.MYANMAR_SHOWS;
    ...
    public final Logger _debug_log = Logger.getLogger("Debug_service");

    public MyanmarShowsBotHandler(int source) {
        getListBot(source);
    }
    ...
    public void getUserGame(int mark, short gameId, int tableId, int Diamond) {
        synchronized (listBots){
            try{
                short type = ShowsConstant.getMarkBotConfigByMark(mark).getType();
                ...
                while (iterator.hasNext()){
                    ...
                    if(u.getGameid() == gameId &&
                        ...
                        long addAG = 0;
                        if (u.getAG() < ShowsConstant.getMarkCreateTableByMark(mark).getAg() || u.getAG() > mark * MAX_AG_RATE) {
                            addAG = getAGADD(type);
                            if (addAG > 0) {
                ...
                _debug_log.info("getUserGame: "+listBots.size() + " "+ dicBots.size());
            }
        }
    }
    ...
    public long getAGADD(short type) {
        return ActionUtils.randomBetween2Number(
                ShowsConstant.getMarkCreateTableByMark((int)ShowsConstant.getMarkBotConfigByType(type).getMark()).getMark() * 200
                , 400 * ShowsConstant.getMarkCreateTableByMark((int)ShowsConstant.getMarkBotConfigByType(type).getMark()).getMark());
    }


    public int getVip(short type) {
        MarkBotConfig markBotConfig = ShowsConstant.getMarkBotConfigByType(type);
        ...
        try {
            synchronized (listBots){
                ...
                while (iterator.hasNext()){
                    ...
                    if(u.getGameid() == gameid
                            && u.getUsertype() == type && u.getTableId() == 0){
                        if (u.getAG() < ShowsConstant.getMarkCreateTableByMark(mark).getAg()) { // kiem tra them tien cho bot
                        ...
                        } else if (u.getAG() < ShowsConstant.getMarkCreateTableByMark(mark).getAg()) {
            ...
            _debug_log.info("BotCreateTable: "+listBots.size() + " "+ dicBots.size());
        }
    }
    ...
    public UserInfo updateBotOnline(int pid, short isOnline) {
        try {
            synchronized (listBots) {
                ...
                _debug_log.info("updateBotOnline: "+listBots.size() + " "+ dicBots.size());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void BotCreateTableForRoom(int gameid, int mark, int roomID) {
        try {
            ...
            _debug_log.info("BotCreateTableForRoom: "+listBots.size() + " "+ dicBots.size());
        }
    }
    ...
    public void getListBot(int source) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                count++;
                userTemp.setUsertype(ShowsConstant.getUserTypeByCountBot(sumBot, count));
            ...
            _debug_log.info("getListBot: "+listBots.size() + " "+ dicBots.size());
        }
    }
    ...
    public void processBotDisconnect(int playerId) {
        try {
            ...
            _debug_log.info("processBotDisconnect: "+listBots.size() + " "+ dicBots.size());
        }
    }
    ...
    public UserInfo processConfirmRoom(int pid, int roomId, int tableId, int mark) {
        try {
            ...
            _debug_log.info("processConfirmRoom: "+listBots.size() + " "+ dicBots.size());
        }
        return null;
    }
    ...
    public String processGetBotInfoByPid(int pid, int tid) {
        try {
            ...
            _debug_log.info("processGetBotInfoByPid: "+listBots.size() + " "+ dicBots.size());
        }
        return "";
    }
    ...
    public void botRejectJoinTable(int pid) {
        synchronized (listBots){
            try{
                ...
                _debug_log.info("botRejectJoinTable: "+listBots.size() + " "+ dicBots.size());
            }
        }
    }
}
