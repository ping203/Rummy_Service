...
public class StreetHiloBotHandler implements Bot{
    ...
    public ConcurrentLinkedHashMap<Integer, UserInfo> dicBotStreetHilo
            = ConcurrentLinkedHashMap.create(ConcurrentLinkedHashMap.EvictionPolicy.SECOND_CHANCE, 1000);
    public List<UserInfo> listBotStreetHilo = new ArrayList<UserInfo>();
    ...
    public final Logger logAgBot =  Logger.getLogger("AddAGBotStreethilo");
    public StreetHiloBotHandler(int source) {
		getListBot(source);
	}
...
//        ==>StreetHiloBotHandler==>getUserGame==>getBot 0 - 100 - 8027 - 1

//        System.out.println("==>StreetHiloBotHandler==>getUserGame==>getBot: " + dicBotStreetHilo.size() + " - " + minAG + " - " + gameId + " - " + tableId);
        synchronized (listBotStreetHilo) {
            try {
                short type = getTypeByMark(minAG);

                //boolean check = true;
                for (int i = 0; i < listBotStreetHilo.size(); i++) {
                   //System.out.println("Bot type=" + listBotStreetHilo.get(i).getUsertype() + "-type" + type+" - "+listBotStreetHilo.get(i).getGameid() );
                    if ((listBotStreetHilo.get(i).getGameid() == gameId || listBotStreetHilo.get(i).getGameid() == 8037)// client bot connected
                            && listBotStreetHilo.get(i).getUsertype() == type) { // check type + ag

                        UserInfo u = listBotStreetHilo.get(i);
                        //System.out.println("listBotStreetHilo aaa -minAg" + lsMinAg.get((int) u.getUsertype()));
                        //System.out.println("listBotStreetHilo"+ ActionUtils.gson.toJson(listBotStreetHilo.get(i)));
                        int addAG = 0;
...
//                        System.out.println("Bot Ag=" + u.getAG() +"-Agadd="+ addAG);
                        if (addAG > 0) {
                            ...
                            ActionUtils.updateAGBOT(u, u.getAG().intValue(),abDB, GAMEID.STREETHILO);
                            Logger.getLogger("StreetHiloBot_Add_Gold").info(ActionUtils.gson.toJson(u) + " - " + minAG + " - " + addAG);
                        }
//                        System.out.println("Bot detected = 11 "  );
                        u.setIsOnline(gameId);
//                        System.out.println("Bot detected id= "+ u.getUserid()  );
                        dicBotStreetHilo.put(u.getUserid(), u);
//                      System.out.println("Bot size "+ dicBotStreetHilo.size()  +"-uid="+ u.getUserid() );
...
//                        System.out.println("Bot detected =  " + u.getAG());
                ...
                //if (check) {
                 //   Logger.getLogger("Bot_Details").info("getUserGame failed: minAG:" + minAG + " - List:" + listBotStreetHilo.size());
                //}
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public UserInfo updateBotOnline(int pid, short isOnline) {
        try {
            synchronized (listBotStreetHilo) { 
		            if (isOnline == 0) {
		                ...
		                if (dicBotStreetHilo.containsKey(pid) && dicBotStreetHilo.get(pid).getIsOnline() > 0) {
		                    dicBotStreetHilo.get(pid).setIsOnline(isOnline);
		                    dicBotStreetHilo.get(pid).setTableId(0);
		                    if(dicBotStreetHilo.get(pid).isDisconnect()) dicBotStreetHilo.get(pid).setGameid((short)0);
		                    UserInfo u = dicBotStreetHilo.get(pid);
		                    listBotStreetHilo.add(u);
		                    dicBotStreetHilo.get(pid).setRoomId((short)0);
		                }
		            } else {
		                dicBotStreetHilo.get(pid).setIsOnline(isOnline);
		            }
        ...
        return dicBotStreetHilo.get(pid);
    }
    

    public Long UpdateBotMarkChessByName(int uid, int source, String name, long mark) {
        synchronized (dicBotStreetHilo) {
//             System.out.println("UpdateBotMarkChessByName:" + uid+"-"+name+"-"+mark);
            if (dicBotStreetHilo.containsKey(uid)) {
//                System.out.println("ag:" +dicBotStreetHilo.get(uid).getAG());
                try {
                    long ag = dicBotStreetHilo.get(uid).getAG() + mark;
                    if (ag < 0) {
                        ag = 0;
                    }
                    dicBotStreetHilo.get(uid).setAG(ag);
                    ...
                    return dicBotStreetHilo.get(uid).getAG();
                } catch (Exception e) {
                    e.printStackTrace();
...
//                type = 33;
//            }
//            System.out.println("==>StreetHiloHandler==>BotCreateTable : " + type +"-gameid"+ gameid);
            //boolean check = true;
            for (int i = 0; i < listBotStreetHilo.size(); i++) {
//                System.out.println("Room"+ listBotStreetHilo.get(i).getRoomId());
                if (listBotStreetHilo.get(i).getGameid() == gameid && listBotStreetHilo.get(i).getUsertype() == type && listBotStreetHilo.get(i).getRoomId() == 0) {
                    //check = false;                
                    UserInfo u = listBotStreetHilo.get(i);
...
//                        System.out.println("AG ADd = " + addAG);
                    ...
                    listBotStreetHilo.get(i).setRoomId((short) ((new Random()).nextInt(2) + 1));
...
//            if (check) {
//                for (int i = 0; i < listBotStreetHilo.size(); i++) {
//                    if (listBotStreetHilo.get(i).getUsertype() == type) {
//                        Logger.getLogger("Bot_Details").info("==>BotHandler==>HadBotCreateTable: failed listBotNew size: " + listBotStreetHilo.size()
...
//					httpclient.close();
    ...
    public void checkBot(JsonObject je, UserInfo actionUser) {
        try {
            ...
            for (int i = 0; i < listBotStreetHilo.size(); i++) {
                if (listBotStreetHilo.get(i).getGameid() == 0) {
                    s++;
                }
                if (listBotStreetHilo.get(i).getUsertype() > 32) {
                    t33++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 31) {
                    t32++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 30) {
                    t31++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 22) {
                    t23++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 21) {
                    t22++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 20) {
                    t21++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 11) {
                    t12++;
                } else if (listBotStreetHilo.get(i).getUsertype() > 10) {
                    t11++;
                }
                if (je.has("T") && je.get("T").getAsInt() == listBotStreetHilo.get(i).getUsertype()) {
                    lsBot.add(listBotStreetHilo.get(i).getUsername());
                }
            }
            JsonObject jo = new JsonObject();
            jo.addProperty("evt", "checkBot");
            jo.addProperty("Disconnected", s);
            jo.addProperty("IsOnline", "Size: " + listBotStreetHilo.size() + " - " + t33 + "/" + t32 + "/" + t31 + "/" + t23 + "/"
                    + t22 + "/" + t21 + "/" + t12 + "/" + t11);
    ...
    public void reloadBot(JsonObject je, UserInfo actionUser) {
        try {
            ...
            for (int i = 0; i < listBotStreetHilo.size(); i++) {
                ...
                if (listBotStreetHilo.get(i).getIsOnline() > 0) {
                    t++;
                }
            }
            ...
            jo.addProperty("Disconnected", s);
            jo.addProperty("IsOnline", t + "/" + listBotStreetHilo.size());
            ClientServiceAction csa = new ClientServiceAction(actionUser.getPid(), 1, ActionUtils.gson.toJson(jo).getBytes("UTF-8"));
            serviceRouter.dispatchToPlayer(actionUser.getPid(), csa);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void getListBot(int source) {
        ...
        try {
            ...
            cs.setInt("Gameid",GAMEID.STREETHILO);
...
//	            	
            	...
            	ActionUtils.updateAGBOT(userTemp, ag, agDB, GAMEID.STREETHILO);
                listBotStreetHilo.add(userTemp);
            }
            rs.close();
            cs.close();
           
            System.out.println("==>Size listBotStreetHilo:" + listBotStreetHilo.size());
	...
	public UserInfo botLogin(int gameid) {
		synchronized (listBotStreetHilo) {
			for (int i = 0; i < listBotStreetHilo.size(); i++) {
				if (listBotStreetHilo.get(i).getGameid() == 0) {
					listBotStreetHilo.get(i).setGameid((short) gameid);
					return listBotStreetHilo.get(i);
				}
			}
		}
		return null;
	...
	public void processBotDisconnect(int playerId) {
		try{
			for (int i = 0; i < listBotStreetHilo.size(); i++) {
				if (listBotStreetHilo.get(i).getPid() == playerId) {
					listBotStreetHilo.get(i).setGameid((short) 0);
					break;
				}
			}
			if (dicBotStreetHilo.containsKey(playerId)) {
				dicBotStreetHilo.get(playerId).setDisconnect(true);
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
	...
	public UserInfo processConfirmRoom(int pid, int roomId, int tableId, int mark) {
		try{
			if(!dicBotStreetHilo.containsKey(pid)) return null;
			dicBotStreetHilo.get(pid).setRoomId((short) roomId);
			dicBotStreetHilo.get(pid).setTableId(tableId);
			dicBotStreetHilo.get(pid).setAS((long) mark);
			for (int i = 0; i < listBotStreetHilo.size(); i++) {
				if (listBotStreetHilo.get(i).getPid() == pid) {
					listBotStreetHilo.remove(i);
					break;
				}
			}
			return dicBotStreetHilo.get(pid);
	...
	public String processGetBotInfoByPid(int pid, int tid) {
		try{
			synchronized (dicBotStreetHilo) {
				if (dicBotStreetHilo.containsKey(pid)) {
					if (tid > 0) {
						dicBotStreetHilo.get(pid).setTableId(tid);
					}
					return ActionUtils.gson.toJson(dicBotStreetHilo.get(pid).getUserGame());
				}
			}
		}catch (Exception e) {
			e.printStackTrace();
	...
	public ConcurrentLinkedHashMap<Integer, UserInfo> getDicBot() {
		return dicBotStreetHilo;
	}

	@Override
	public List<UserInfo> getListBot() {
		return listBotStreetHilo;
	}
