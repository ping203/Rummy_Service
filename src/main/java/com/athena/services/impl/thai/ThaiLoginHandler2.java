...
public class ThaiLoginHandler2 implements LoginHandler {
    ...
    public ThaiLoginHandler2(ServiceContext context) {
    ...
    public LoginResponseAction handle(LoginRequestAction action) {
    	...
    	try {
    		//Thread.sleep(5000);
    		loggerLogin_.info("==>StartLogin Thai2:" + (new Date()).toString() +": "+ action.getUser() + "---" + action.getOperatorid() + "-" + action.getPassword());        	
            ...
            int userid = uController.GameGetUseridSiam(ServerSource.THAI_SOURCE, ulogin.getUserIdZing(), ulogin.getDeviceId()) ;
