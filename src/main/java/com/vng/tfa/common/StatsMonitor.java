		...
		if (history != null && history.length > 0)
		{
			...
			for (int i = 0; i < history.length; i++)
			{
				if(history[i] != 0)
				{
					...
					ratio = (ratio) / total;
					if (i < bucketoffset.length)
					{
						content += "<li>< " + formatter.format((double) bucketoffset[i] / 1000) + " msec : \t" + history[i] + "\t" + displayProgressBar(ratio) + "</li>";
					}
					else
					{
						content += "<li>" + "> " + bucketoffset[bucketoffset.length - 1] + ":" + history[i] + "</li>";
					}
				}
			}
			content += "</ul>";
